...
class PoseDetector(object):
    ...
    def compute_optimal_size(self, orig_img, img_size, stride=8):
        ...
        aspect = float(orig_img_h) / orig_img_w
    ...
    def __call__(self, orig_img):
        ...
        if len(all_peaks) == 0:
            return np.empty((0, len(JointType), 3)), np.empty(0)
        peaks = []
        #all_peaks_thre = all_peaks[all_peaks[:,3] > 0.01]
        for joint_id in np.unique(all_peaks[:,0]):
            #peak = all_peaks_thre[np.argmax(all_peaks_thre[all_peaks_thre[:,0] == joint_id][:,3])]
            peak = all_peaks[all_peaks[:,3] == all_peaks[all_peaks[:,0] == joint_id][:,3].max(axis=0)]
            peaks.append(peak[0])
        ...
        all_peaks_max = np.array(peaks).reshape(-1,5)
        all_peaks_max[:, 1] *= orig_img_w / map_w
        all_peaks_max[:, 2] *= orig_img_h / map_h
        #poses = self.subsets_to_pose_array(subsets, all_peaks)
        poses = np.zeros((10,3))

        for ind in np.unique(all_peaks[:,0]):
            ind = ind.astype('i')
            poses[ind][0:2] = all_peaks_max[ind == all_peaks_max[:,0]][0][1:3]
            poses[ind][2] = 2
        #scores = subsets[:, -2]
        scores = all_peaks_max[:,3]
        return poses, scores, heatmaps
...
if __name__ == '__main__':
    ...
    cv2.imwrite('tmp_result.png', tmp_img)
